...
def client_gui():
    import neverblock_client
    from neverblock_client import app
    parser = optparse.OptionParser()
    parser.add_option('--version', action='store_true', help='Print version')
    (options, args) = parser.parse_args()

    if options.version:
        print '%s v%s' % (neverblock_client.__title__, neverblock_client.__version__)
    else:
        client_app = app.App()
        client_app.main()

def client_shell():
    from neverblock_client import constants
    constants.set_shell()
    ...
    def daemon_cmd(pidfile, foreground):
        ...
        from neverblock_client import shell_app
        shell_app.ShellApp()
    ...
    try:
        ...
        if passwd:
            with open(pass_path, 'w') as passwd_file:
                ...
                passwd_file.write('neverblock_client\n')
                passwd_file.write('%s\n' % passwd)
...
def pk_stop():
    ...
    with open('/proc/%s/cmdline' % pid, 'r') as cmdline_file:
        ...
        if not 'neverblock-client-pk-start' in cmdline and \
                not 'neverblock-client-pk-autostart' in cmdline:
            raise ValueError('Not a neverblock client process')
...
def pk_clear_autostart():
    profile_hash_path = os.path.join(os.path.abspath(os.sep),
        'etc', 'neverblock_client', sys.argv[1])
