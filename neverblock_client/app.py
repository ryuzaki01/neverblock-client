...
class App(object):
    ...
    def update_menu(self):
        ...
        if not profile_count:
            menu_item = interface.MenuItem()
            menu_item.set_label('Please Login to Connect')
        ...
        menu_item = interface.MenuItem()
        menu_item.set_label('Login')
        menu_item.set_callback(self.show_login)
        menu.add_item(menu_item)
    ...
    def wait_for_usb_insert(self):
        ...
        dialog.set_message_secondary(
            'Insert Neverblock USB key to unlock profile')
        dialog.run()
        dialog.destroy()
    ...
    def wait_for_usb_remove(self):
        while True:
            ...
            dialog.set_message_secondary('Remove Neverblock USB key')
            dialog.run()
            dialog.destroy()
    ...
    def on_setup_usb_key(self, profile_id):
        ...
        if not profile.has_usb_device():
            ...
            if not profile.has_usb_device():
                ...
                dialog.set_message_secondary(
                    'Insert existing Neverblock USB key or select a USB ' +
    ...
    def show_about(self):
        from neverblock_client import __version__
        dialog = interface.MessageDialog()
        dialog.set_type(MESSAGE_INFO)
        dialog.set_buttons(BUTTONS_OK)
        dialog.set_title(APP_NAME_FORMATED)
    ...
    def show_import_profile(self):
        ...
        dialog.set_title(APP_NAME_FORMATED)
        dialog.set_icon(utils.get_logo())
        dialog.add_filter('Neverblock Profile', '*.ovpn')
        dialog.add_filter('Neverblock Profile', '*.conf')
        dialog.add_filter('Neverblock Profile', '*.tar')
