...
class Page:
    ...
    def build_page(self):
        if not self._type or not self._buttons:
            return
        self._page = gtk.Window(gtk.WINDOW_TOPLEVEL)
        self._page.connect("delete_event", self.delete_event)
        self._page.connect("destroy", self.destroy)
        self._page.set_border_width(1)
        self._page.resize(200, 400)

    def set_bg(self, bg_path):
        self._bg_image = bg_path
        if self._page:
            pixbuf = gtk.gdk.pixbuf_new_from_file(self._bg_image)
            pixmap, mask = pixbuf.render_pixmap_and_mask()
            self._page.set_back_pixmap(pixmap, gtk.FALSE)
        else:
            self.build_page()
    ...
    def run(self):
        self._page.show_all()
        self._page.set_keep_above(True)
        self._page.set_keep_above(False)
        response = self._page.run()
